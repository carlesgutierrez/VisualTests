...
//------------------------------------------------------------------
LightBrushParticle::LightBrushParticle(){
	nodePoints = NULL;
}

//------------------------------------------------------------------
void LightBrushParticle::setMode(lightBrushParticleMode newMode){
	mode = newMode;
}

//------------------------------------------------------------------
void LightBrushParticle::setAttractPoints(vector <ofPoint> * _nodePoints) {
	nodePoints = _nodePoints;
}

//------------------------------------------------------------------
void LightBrushParticle::setAttractPointsSizes(vector <int> * _nodePointsSizes) {
	nodeSizes = _nodePointsSizes;	
}

//------------------------------------------------------------------
void LightBrushParticle::setPerlinParameters(float _param2d, float _param3d) {
	perlinNoiseParam2d = _param2d;
	perlinNoiseParam3d = _param3d;
}

//------------------------------------------------------------------
void LightBrushParticle::reset(){
	//the unique val allows us to set properties slightly differently for each particle
	uniqueVal = ofRandom(-1000, 1000);//TODO checking this ranges 
	
	ofPoint ranDomPos = ofPoint(ofRandom(10, LIGTHLAMP_SIZE_WIN_W - 10), ofRandom(10, LIGHTLAMP_SIZE_WIN_H - 10));
	pos = ranDomPos;
	...
	if( mode == LIGHTBRUSH_PARTICLE_MODE_NOISE_SNOW_DIR){
...
//------------------------------------------------------------------
void LightBrushParticle::resetRounComplete() {
	bRoundComplete = false;
}

//------------------------------------------------------------------
int LightBrushParticle::getIdNextNode() {
	return idNextNode;
}

//------------------------------------------------------------------
ofPoint LightBrushParticle::updateDirToGo() {
...
//------------------------------------------------------------------
void LightBrushParticle::update(){

	//1 - APPLY THE FORCES BASED ON WHICH MODE WE ARE IN 
	
	if( mode == LIGHTBRUSH_PARTICLE_MODE_NOISE_WALKER){
	...
	else if( mode == LIGHTBRUSH_PARTICLE_MODE_NOISE_SNOW_DIR){
		...
		//Y borders
		if (pos.y + vel.y > LIGHTLAMP_SIZE_WIN_H) {
			pos.y -= LIGHTLAMP_SIZE_WIN_H;
		}

		//X borders
		if (pos.x + vel.x > LIGTHLAMP_SIZE_WIN_W) {
			pos.x -= LIGTHLAMP_SIZE_WIN_W;
		}
